...
export const placeOrderService = async (
    ...
    for (const cartItem of cart.items) {
        ...
        if (!variant) throw new Error(`${product.name} variant not found`);
        if (variant.stock < cartItem.quantity) throw new Error(`${product.name} stock not available`);
...
export const applyCouponService = async (code, userId, session) => {
    ...
    if (now < new Date(coupon.startDate)) return { success: false, message: "This coupon is not valid yet" };
    if (now > new Date(coupon.endDate)) return { success: false, message: "This coupon has expired" };
    if (coupon.usedCount >= coupon.usageLimit) return { success: false, message: "This coupon has reached its usage limit" };
    ...
    if (coupon.minOrder > 0 && totalAmount < coupon.minOrder) {
        return { success: false, message: `Minimum order ₹${coupon.minOrder}` };
    }
...
export const removeCouponService = async (session) => {
    if (!session.appliedCoupon) return { success: false, message: "No coupon applied" };
